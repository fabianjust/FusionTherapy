function [subject] = mvc_glidingWindow(vararin,mvcs) % add subject
...
for i = 1:length(mvcs)
    ...
    % 3. Define gliding window/moving mean
    w_size = fs*0.5; % window size for a duration of 500 ms -> The ABC of EMG, P. Konrad, 2005
    mov_mean = movmean(EMG_envelope,w_size);
    max_mean = max(mov_mean);
    mvc_mean_all(1,k) = max_mean; % save mvc mean of a single repetition
    end
